...
export type HostFileKind = "directory" | "file" | "roots";
...
function hostFileQuery(path: string, extra: Record<string, string> = {}) {
  // An empty path is the fenced entry point: the API returns the allowed roots.
  const params = new URLSearchParams({ ...(path ? { path } : {}), ...extra });
