...
// The empty path is the fenced entry point listing the allowed roots.
function breadcrumbs(path: string) {
  const roots = { path: "", label: "Allowed roots" };
...
function hostErrorMessage(error: unknown) {
  if (error instanceof HostFileRequestError) {
    if (error.code === "path_not_allowed") return "This path is outside the allowed roots (Agent workspaces and the Loom data directory).";
...
export function HostFilesPane({ initialPath = "", onOpenHostFile }: { initialPath?: string; onOpenHostFile?: (path: string) => void }) {
  ...
  const directory = node?.kind === "directory" || node?.kind === "roots" ? (node as HostDirectory) : null;
    ...
    <main className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden bg-background" aria-label="Host files">
      <div className="shrink-0 border-b border-border px-4 py-3 md:px-6">
        <div className="mx-auto max-w-[1080px] space-y-2">
          <DirectoryToolbar path={path || "Allowed roots"} loading={loading} onRefresh={refresh} />
