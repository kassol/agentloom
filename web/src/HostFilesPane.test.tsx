...
describe("HostFilesPane", () => {
  ...
  it("opens the allowed-roots listing when no path is given, without a path parameter", async () => {
    const fetchMock = vi.fn().mockResolvedValue(jsonResponse({
      ...
      readable: true,
      entries: [
        { path: "/tmp/agent-workspace", name: "agent-workspace", kind: "directory", size: 0, modifiedAt: "2026-08-14T08:00:00Z", mode: "drwx------", readable: true },
      ],
    }));
    vi.stubGlobal("fetch", fetchMock);

    render(<HostFilesPane />);
    expect(await screen.findByText("agent-workspace")).toBeInTheDocument();
    const [firstURL] = fetchMock.mock.calls[0];
    expect(String(firstURL)).not.toContain("path=");
  });

  it("explains a fenced path instead of showing a generic error", async () => {
    const fetchMock = vi.fn().mockResolvedValue(jsonResponse({ error: { code: "path_not_allowed", message: "path is outside the allowed roots" } }, 403));
    vi.stubGlobal("fetch", fetchMock);

    render(<HostFilesPane initialPath="/etc" />);
    expect(await screen.findByRole("alert")).toHaveTextContent("This path is outside the allowed roots (Agent workspaces and the Loom data directory).");
  });
