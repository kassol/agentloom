...
// allowedFileRoots limits host file access to directories Loom already
// governs: every Agent's working directory plus the Loom data directory.
// Collected per request so newly created Agents are visible immediately.
func (s *Server) allowedFileRoots() []string {
	var roots []string
...
// resolveAllowedFilePath resolves symlinks and rejects any path that does not
// live under an allowed root, so a symlink inside a root cannot escape it.
func (s *Server) resolveAllowedFilePath(path string) (string, error) {
	resolved, err := filepath.EvalSymlinks(path)
	if err != nil {
		return "", classifyFileError(err, "path")
	}
	for _, root := range s.allowedFileRoots() {
	...
	return "", &fileHTTPError{status: http.StatusForbidden, code: "path_not_allowed", message: "path is outside the allowed roots"}
}

func (s *Server) fileRoots(w http.ResponseWriter) {
	listing := fileListing{Kind: "roots", Readable: true}
	roots := s.allowedFileRoots()
	listing.Entries = make([]fileEntry, 0, len(roots))
	for _, root := range roots {
		info, err := os.Stat(root)
		if err != nil {
			listing.Entries = append(listing.Entries, fileEntry{
				Name: filepath.Base(root), Path: root, Kind: "directory",
				Readable: false, ErrorCode: classifyFileCode(err),
			})
			continue
		}
		listing.Entries = append(listing.Entries, describeFile(root, filepath.Base(root), info))
	}
...
func (s *Server) fileMetadata(w http.ResponseWriter, r *http.Request) {
	...
	path, err := s.allowedFilePathFromQuery(r)
	if err != nil {
...
func (s *Server) allowedFilePathFromQuery(r *http.Request) (string, error) {
	path, err := absoluteFilePath(r.URL.Query().Get("path"))
	if err != nil {
		return "", err
	}
	return s.resolveAllowedFilePath(path)
}
